...
load-zeebe:
	@docker pull dddpaul/zeebe-speedb:8.4.5
	@kind load docker-image dddpaul/zeebe-speedb:8.4.5 --name ${CLUSTER} --nodes ${CLUSTER}-worker2,${CLUSTER}-worker3,${CLUSTER}-worker4,${CLUSTER}-worker5
	@docker pull camunda/operate:8.4.4
	@kind load docker-image camunda/operate:8.4.4 --name ${CLUSTER} --nodes ${CLUSTER}-worker6
...
load-connectors:
	@docker pull camunda/connectors-bundle:8.4.5
	@kind load docker-image camunda/connectors-bundle:8.4.5 --name ${CLUSTER} --nodes ${CLUSTER}-worker10
